...
dbase = os.path.relpath(os.path.normpath(os.path.join(os.path.dirname(__file__), '../../data/nx=3^6x2')))
N = 2 * (3 ** 6)
...
for name, stress in zip(stress_names, stresses):
    ...
    with h5py.File(os.path.join(dbase, 'AvalancheAfterPush', '{0:s}.hdf5'.format(name)), 'r') as data:
