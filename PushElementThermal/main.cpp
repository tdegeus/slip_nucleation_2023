...
class Main : public FQF::BrownianThermalHybridSystem {
...
public:
    ...
    {
        H5Easy::dump(m_file, "/completed", code, H5Easy::DumpMode::Overwrite);
    }
...
public:
    ...
    int run_push(const std::string& outfilename)
    {
        ...
        this->setKBT(H5Easy::load<double>(m_file, "/push/kBT"));
        ...
        H5Easy::dump(data, "/meta/push/kBT", H5Easy::load<double>(m_file, "/push/kBT"));
...
static const char USAGE[] =
    R"(PushElementThermal
    Push a random element and keep a finite temperature until a target stress has been reached,
    then compute force equilibrium.
...
int main(int argc, const char** argv)
{
    ...
    for (size_t i = 0; i < 100; ++i) {
        ...
        // stop if the run was success-full
        if (ret == 1) {
            fmt::print("Complete : {0:s}\n", outname);
            sim.write_completed(1);
            return 0;
        }
    }

    sim.write_completed(0);
