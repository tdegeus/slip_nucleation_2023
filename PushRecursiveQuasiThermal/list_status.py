...
for file in basefiles:
    ...
    if len(ondisk) > 0:
        ...
        if completed == 1 and np.array_equal(ondisk, stored):
            output['completed_base'] += [file]
            output['completed_event'] += basefiles[file]
        elif completed == -1 and np.array_equal(ondisk, stored):
            output['stopped_base'] += [file]
            output['stopped_event'] += basefiles[file]
