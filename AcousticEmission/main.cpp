...
void run(double stress, size_t element, size_t inc_c, const std::string& output)
{
  ...
  // allocate signal
  std::vector<double> sig_xy_b;
  std::vector<double> sig_xy_c;
  std::vector<double> sig_xy_e;

  // quench: force equilibrium
  for (size_t iiter = 0; ; ++iiter)
  {
    // - store
    sig_xy_b.push_back(m_Sig(0      ,0,0,1));
    sig_xy_c.push_back(m_Sig(100    ,0,0,1));
    sig_xy_e.push_back(m_Sig(element,0,0,1));
  ...
  H5Easy::dump(data, "/sig_xy/boundary", sig_xy_b);
  H5Easy::dump(data, "/sig_xy/close", sig_xy_c);
  H5Easy::dump(data, "/sig_xy/plastic", sig_xy_e);
