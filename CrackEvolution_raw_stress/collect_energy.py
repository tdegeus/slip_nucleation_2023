...
def main():
    ...
    with h5py.File(output, "w") as out:
        ...
        for key in A_val:
            ...
            out[f"/A/{key:s}"] = mean[keep]
            out[f"/A/{key:s}"].attrs["norm"] = norm[keep]
            out[f"/A/{key:s}"].attrs["first"] = first[keep]
            out[f"/A/{key:s}"].attrs["second"] = second[keep]
        ...
        for key in t_val:
            ...
            keep = norm >= 0.9 * np.max(norm)
            out[f"/t/{key:s}"] = mean[keep]
            out[f"/t/{key:s}"].attrs["norm"] = norm[keep]
            out[f"/t/{key:s}"].attrs["first"] = first[keep]
            out[f"/t/{key:s}"].attrs["second"] = second[keep]
