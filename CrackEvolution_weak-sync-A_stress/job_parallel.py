...
def get_runs(name, stress):
    ...
    lines = ['./CrackEvolution_stress --file {file:s} --element {element:d} --incc {incc:d} --stress {stress:.8e} --output {output:s}'.format(**c) for c in commands]
...
for name, stress in zip(stress_names, stresses):
    ...
    sbatch = {
        'job-name'      : 'velocity_{0:s}'.format(name),
        'out'           : 'job.out',
        'nodes'         : 1,
        'ntasks'        : 1,
        'cpus-per-task' : 28,
