...
if not os.path.isdir('stress_middle-layer_all'): os.makedirs('stress_middle-layer_all')
...
for nx in list_nx()[::-1]:
  ...
  for stress in list_stress()[::-1]:
    ...
    if nx == 'nx=3^6': Ac_list = np.linspace(100,  700, 14).astype(np.int)
    else             : Ac_list = np.linspace(100, 1400, 28).astype(np.int)
    ...
    for i, Ac in enumerate(Ac_list):
      ...
      x = np.arange(N)
      y = applyFilter(data['Sig_eq'][2,:])

      ax.plot(x, y, c=cmap(i))

    ax.plot([0, N], [data['sig_c'], data['sig_c']], c='k', ls='--')
    ax.plot([0, N], [data['sig_n'], data['sig_n']], c='k', ls='-.')

    plt.savefig('stress_middle-layer_all/{nx:s}_{stress:s}.pdf'.format(nx=nx, stress=stress))
    plt.close()
