...
if not os.path.isdir('stress_middle-layer'): os.makedirs('stress_middle-layer')
...
for nx in list_nx()[::-1]:
  ...
  for stress in list_stress()[::-1]:
    ...
    plt.savefig('stress_middle-layer/{nx:s}_{stress:s}.pdf'.format(nx=nx, stress=stress))
    plt.close()
